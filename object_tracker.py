...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if scores != []:
                score = scores[current_track]
            else:
                score = 0

            n_score = round(score*100, 1)
            bbox_mess = '%s %s %s' % (class_name, str(track.track_id), n_score)
        ...
        # draw bbox on screen
            ...
            # box around the description
            cv2.rectangle(frame, (int(bbox[0]), int(bbox[1]-30)), (int(bbox[0])+(len(class_name)+len(str(track.track_id))+len(str(n_score)))*17, int(bbox[1])), color, -1)
